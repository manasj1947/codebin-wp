<?php include('header.php')?>
  <!-- PAGE TITLE LARGE 5 ABSOLUTE GRAY -->
      <div class="page-title-large5-cont bg-gray">
        <div class="container relative">
          <div class="page-title-large5-text-cont">
            <div>
              <h1 class="page-title5">Fifth Season</h1>
            </div>
          </div>
        </div>
      </div>

      <!-- CONTENT -->
      <div class="page-section p-140-cont pb-50">
        <div class="container">

          <!-- PROJECT DETAILS & IMG -->
          <div class="row">

            <!-- PROJECT DETAILS -->
            <div class="col-sm-4 col-md-3 mb-50">

              <!-- PROJECT DETAILS -->
              <div class="port-detail-cont mb-30">

                <div class="port-detail font-poppins mb-20">
                  <p>
                    <strong class="font-black">Client:</strong>
                    Fifth Season
                  </p>
                  <p>
                    <strong class="font-black">Products:</strong>
                    Online Food Ordering and Delivery
                  </p>
                  <p>
                    <strong class="font-black">Description:</strong>
                    Singapores Prime Tangra Chinese Food delivery restaurant
site and app
                  </p>
                  <p>
                    <strong class="font-black">Skills:</strong>
                    WordPress, PHP, Rest API
                  </p>
                </div>

              </div>

            </div>

            <!-- IMG -->
            <div class="col-sm-8 col-md-8 col-md-offset-1 mb-50">
              <div class="lightbox-item popup-gallery">
                <a href="images/img/products/single/FifthSeason.jpg">
                  <div class="port-img-overlay">
                    <img class="port-main-img" src="images/img/products/single/FifthSeason.jpg" alt="img">
                  </div>
                  <div class="port-overlay-cont">
                    <div class="port-btn-cont">
                      <div aria-hidden="true" class="icon_search"></div>
                    </div>
                  </div>
                </a>
              </div>
            </div>

          </div>

        </div><!-- container end -->

        <!-- DIVIDER -->
        <hr class="mt-0 mb-0">

        <!-- WORK NAVIGATION -->
        <div class="row">
          <div class="col-md-12">
            <div class="work-navigation plr-10 clearfix">
              <a href="disney-london.php" class="work-prev"><span><span class="icon icon-arrows-left"></span>&nbsp;Prev</span></a>
              <a href="./#portfolio" class="work-all"><span>All</span></a>
              <a href="learnthings-africa.php" class="work-next"><span>Next&nbsp;<span class="icon icon-arrows-right"></span></span></a>
            </div>
          </div>
        </div>
      </div><!-- page section end -->

<?php include('footer.php')?>